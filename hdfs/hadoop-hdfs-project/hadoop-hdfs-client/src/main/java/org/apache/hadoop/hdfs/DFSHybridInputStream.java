...
//            chosenNode.info, System.currentTimeMillis() - startTime);
          ...
          if (chosenNode != null) {
            ...
            if (!triedStripe) {
              ...
              stripeRequest = hedgedService.submit(
                  getFromStripe(stripe, start, end, bb, corruptedBlocks));
  ...
  private void readStripeBlock(LocatedStripedBlock block, long start, long end,
    ...
    try {
      // TODO: import patch from dfsstripedinputstream to fix readTo
      long readTo = 0;
      for (StripedBlockUtil.AlignedStripe stripe : stripes) {
        readTo += stripe.getSpanInBlock();
      }
      ...
      DFSClient.LOG.info("Completed hybrid striped read | full degraded mode = {}, missing = {} units, striped read in {} ms",
          degradedRead, missingUnits, System.currentTimeMillis() - startTime);
    }
  }
  ...
  private Callable<ByteBuffer> getFromStripe(
      ...
      final ByteBuffer bb, final CorruptedBlocks corruptedBlocks) {
    return () -> {
      readStripeBlock(stripe, start, end, bb, corruptedBlocks);
      return bb;
    };
  }
