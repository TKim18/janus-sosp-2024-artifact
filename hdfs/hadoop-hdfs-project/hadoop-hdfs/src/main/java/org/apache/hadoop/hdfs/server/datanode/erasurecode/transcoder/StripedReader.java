...
class StripedReader {
  ...
  DatanodeInfo[] sources; // TODO: won't be final later when this can change for back-ups
  final int[] sourceBlockIdOffsets; // block id's of sources
  final long[] sourceBlockLengths; // block len's of sources
  ...
  StripedReader(StripedTranscoder transcoder, DataNode datanode,
    ...
    this.usingShortcut = transcoderInfo.hasShortcut();
    this.numParities = transcoderInfo.getEcPolicy().getNumParityUnits();
  ...
  private StripedBlockReader createReader(short index) {
  ...
  ByteBuffer[] getInputBuffers4NaiveTranscode(int toTranscodeLen) {
    ByteBuffer[] inputs = new ByteBuffer[sources.length + this.numParities];
    for (int i = 0; i < sources.length; i++) {
      ByteBuffer buffer = readers[i].getReadBuffer();
      paddingBufferToLen(buffer, toTranscodeLen);
      inputs[readers[i].getIndex()] = (ByteBuffer) buffer.flip();
