...
public class DfsPerfSlave {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      int iterations = TestCase.get().getIteration(testCase);
      ...
      for (i = 0; i < iterations; i++) {
        boolean success = task.run(taskContext, i);
        if (!success) break;
        boolean closeFileSystem = (i == iterations - 1);
        success = task.cleanup(taskContext, closeFileSystem) & taskContext.getSuccess();
        if (!success) break;
      }

      if (i == iterations)
        masterClient.slave_finish(taskId, nodeName, true);
      else
